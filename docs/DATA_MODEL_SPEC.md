...
**`Bid.folder_url` (added 2026-08):** a plain pasted-in OneDrive link, not resolved via Graph API. The team creates a **new folder per bid** (not per project) named "bid# - bid name," and renames it to "job# - job name" at award — same folder/link both before and after, so this one field carries through the whole lifecycle rather than needing a separate Job-level field. `getProjectDetail`'s `jobs[]` and `notify.coEmailShape` both surface it via the Job's `winning_bid_id` → that bid's `folder_url`, since the Job's folder IS its winning bid's folder. Set at Start Bid or via Edit Bid; shown as an "Open Folder" link on the bid card, the flyout header, the Job row, and in every notification email that carries a bid/CO shape.

**Jobsite walk-throughs (`Bid.walkthroughs[]`, added 2026-07, changed to a multi-entry array 2026-07):** a bid can have several — different companies, contacts, dates, and times — each its own array entry (`{_id, date, time, company_id, contact_id, reminder_sent}`, `_id` via `nextId('walkthroughs')`) rather than a single set of fields on the bid, so scheduling one doesn't overwrite another. Optional, independent of stage/due_date. Deliberately **not** part of the generic Edit Bid form — its own dedicated add/edit/remove actions (🚶 box in the bid card, one row per walk-through, "+ Add another" to schedule more) since estimating specifically asked for this to be easy to find, not just another field in a long form. Site contact is a real **Company**/**Contact**, found-or-created the same way as any other company/contact picker (`addWalkthrough`/`updateWalkthrough` in `v2/db.js`, via `POST`/`PATCH /api/v2/bids/:id/walkthroughs[/:wid]` rather than the generic admin-edit whitelist, which can't do find-or-create) — not free text, and not necessarily one of the bid's actual customers (often a GC's on-site super rather than an office contact already on the bid). Adding or rescheduling one entry fires an immediate "walk-through scheduled" email (`v2/routes.js` → `notify.notifyWalkthroughSet(bidId, walkthroughId, actorId)`) to the bid's estimator, salesperson, and any sub-estimators, including the resolved site contact's name/company/phone (`notify.walkthroughContactInfo`, now takes one walk-through entry rather than the whole bid). A separate hourly cron (`server.js`) checks every entry across every bid for ones 24 hours out and sends a reminder — this needs BOTH date and time set (no time = nothing to be "24 hours before" of) and only fires once per entry (`reminder_sent`, reset to `false` whenever that entry's date/time changes so a reschedule gets its own fresh reminder; `getBidsNeedingWalkthroughReminder` returns one `{bid, walkthrough}` pair per due entry, and `markWalkthroughReminderSent(bidId, walkthroughId)` targets just that one). Because the reminder is time-sensitive, not just date-sensitive, the "24 hours before" check is done in real UTC-vs-Eastern math (`etWallClockToUTCms` in `v2/db.js`) rather than the naive date-string comparisons used everywhere else in this app — the only place that distinction currently matters. Both emails also carry an "Add to Outlook" button (a real timed Outlook Web event, not all-day). Walk-throughs also show on both calendar views (Dashboard's embedded one and the standalone Calendar page), one chip per entry, scoped to My View the same way due dates/follow-ups already are — no external calendar sync involved, just this app's own calendar.
...
**Editing a logged follow-up (`updateFollowup` in `v2/db.js`, added 2026-07):** a Followup was create-only until now — no path existed to fix a mis-logged one (wrong contact, typo, wrong outcome). Its own `PATCH /api/v2/followups/:id`, not the generic admin-edit whitelist (Followup was never in `ADMIN_EDITABLE` — its parent-rollup side effect doesn't fit that generic `$set` pattern). Permission is simple by design: an admin, or whoever is actually recorded as `contacted_by` on that entry — no bid/CO lookup needed, since `contacted_by` is normally the bid's own estimator/salesperson anyway. If the edited entry is the *most recent* one for its parent, its (possibly just-changed) `next_followup_date` is rolled back up to the parent exactly like `logFollowupV2` does at creation — otherwise a corrected outcome/date on an old entry wouldn't need to touch what the dashboard/calendar currently shows.

**Per-person notification preferences (`TeamMember.notification_prefs`, added 2026-07):** opt-**out**, not opt-in — a missing key or an explicit `true` both mean "send it" (`mailer.wantsNotification(member, category)`), so every account that predates this feature keeps getting exactly what it already got until that person visits Settings and turns something off themselves. Categories: `assigned`, `followup`, `awarded`, `reminder`, `walkthrough`, `digest`, `ideas` (kept in sync between `mailer.js`'s `NOTIFICATION_CATEGORIES` and `public/v2.html`'s client-side copy of the same labels). Self-service only, via `PATCH /api/v2/me/notification-prefs` (`updateMyNotificationPrefs` in `v2/db.js`) — always operates on `req.session.userId`, never a body-supplied id, so there's no path to edit someone else's prefs. Idea/issue *submission* (which goes to one fixed admin inbox, not a roster) is deliberately not a category — there's no "whose inbox" question for it.
