...
| Settings / Team management | ✅ Built, scoped | `/#settings`. **Important discovery while building this:** v1 and v2 are genuinely separate MongoDB databases (`cc-estimating` vs `estimating_v2_test`), not a shared collection as earlier assumed this session — every TeamMember has a *different* id between the two for the same person, and v2's own copies have almost no emails populated (only ever seeded with name/initials/role). This page manages v2's own roster (role/email/active — what actually backs v2's estimator/salesperson dropdowns, filters, and "mine only"). It does **not** create a login; a brand-new person here can be assigned to bids immediately but can't log in until the two rosters are unified (flagged in the page copy). Follow-up timer (`fu_initial_days`/`fu_recurring_days`) and email tools (test email, send-digest-now) also target v2's own Settings doc / v2's `getDigest()`. |
| **Fixed:** cross-database id mismatch (current_user + reminder emails) | ✅ Fixed | The above discovery surfaced two live bugs: (1) `current_user.id` (from v1 login) was compared directly against v2's `bid.estimator_id`/`salesperson_id` for "mine only" — since ids collide on *different* people across the two databases, this could silently show the wrong person's bids. Fixed by name-matching the v1 login to v2's own team list in `GET /api/v2/meta`. (2) The v2 reminder cron looked up email recipients in a v1-id-keyed map using v2 ids as keys. Fixed via `v2/notify.js`'s `emailForV2Member()`, which bridges a v2 team id to a real v1 email by name (the only place real emails currently exist). Both verified against live data after the fix. |
