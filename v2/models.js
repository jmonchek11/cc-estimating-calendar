...
const OutOfOfficeSchema = new mongoose.Schema({
  ...
  // Reminder above. A pre-award bid links via 'bid'; once awarded, either
  // the job itself ('job') or one specific change order on it
  // ('change_order') — job takes priority in search results since a job #
  // is what people actually search by.
  parent_type:    { type: String, enum: ['bid', 'job', 'change_order'], default: null },
