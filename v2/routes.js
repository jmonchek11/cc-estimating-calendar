...
router.get('/api/v2/jobs-pending-all', async (req, res) => {
  try { res.json(await v2db.getAllPendingJobs()); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/api/v2/bids',                   t(async req => {
    // createDirectBid goes straight to active_bid (skips the opportunity
    // stage, and with it the /start route's own notifyAssignmentDiff call)
    // — an estimator/salesperson assigned right here at creation (the normal
    // case for "+ New Bid") was never getting notified. No prior bid to
    // diff against, so any assignment counts as new.
    const r = await v2db.createDirectBid({ ...req.body, created_by: req.session.userId });
    notifyAssignmentDiff(r.bid_id, null, req.body, req.session.userId);
    return r;
  },
...
// A bid can have several walk-throughs — its own dedicated add/update/remove
// actions, not the generic admin PATCH, since scheduling one needs
// find-or-create logic for the site company/contact that the generic
...
// since that middleware reads req.params.entity, which these routes don't have.
async function requireBidAssignedOrAdmin(req) {
  const bid = await v2db.loadBid(req.params.id).catch(() => null);
  const actor = await actorOf(req);
  const allowed = actor?.is_admin || (bid && actor && (bid.estimator_id === actor.id || bid.salesperson_id === actor.id));
  if (!allowed) throw new Error('Only an admin or this bid\'s assigned estimator/salesperson can manage its walk-throughs');
  return bid;
}
router.post('/api/v2/bids/:id/walkthroughs',   t(async req => {
...
router.patch('/api/v2/bids/:id/walkthroughs/:wid', t(async req => {
    const oldBid = await requireBidAssignedOrAdmin(req);
    const oldW = oldBid?.walkthroughs?.find(w => w._id === Number(req.params.wid));
    const r = await v2db.updateWalkthrough(req.params.id, req.params.wid, req.body, req.session.userId);
...
router.delete('/api/v2/bids/:id/walkthroughs/:wid', t(async req => {
    await requireBidAssignedOrAdmin(req);
...
router.post('/api/v2/bids/:id/sub-estimators',   t(async req => {
    ...
    if (req.body.estimator_id) notify.notifyAssigned(Number(req.params.id), Number(req.body.estimator_id), req.session.userId, 'sub_estimator', req.body.scope);
    return r;
  }));
...
router.post('/api/v2/submissions/:id/awaiting-po', t(req => v2db.setAwaitingPo(req.params.id, req.body, req.session.userId)));
...
router.post('/api/v2/followups', async (req, res) => {
  try {
    const contacted_by = req.body.contacted_by || req.session.userId;
    const result = await v2db.logFollowupV2({ ...req.body, contacted_by });
    res.json(result);
    // fire-and-forget — a mail failure must never affect the follow-up itself
    notify.notifyFollowup(req.body.parent_type, req.body.parent_id,
      { notes: req.body.notes, next_followup_date: result.next_followup_date }, contacted_by).catch(() => {});
  } catch (e) { res.status(400).json({ error: e.message }); }
});
...
router.post('/api/v2/change-orders/:id/approve-request', t(async req => {
    const actor = await actorOf(req);
    const r = await v2db.approveCoRequest(req.params.id, req.session.userId, { estimator_id: req.body?.estimator_id, pm_id: req.body?.pm_id });
    notify.notifyCoQueued(r.co_id, actor?.name);
    return r;
  },
...
router.post('/api/v2/change-orders/:id/start', t(req => v2db.startCoRequest(req.params.id, req.body, req.session.userId),
  async (req) => ({ action: 'co.start', summary: `Started CO ${await v2db.coLabel(req.params.id)}`, entity_type: 'change_order', entity_id: Number(req.params.id) })));
